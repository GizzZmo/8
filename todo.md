I created the Python source files described in main.md and placed them under an osi_blockchain_simulation/ package layout. I also added minimal __init__.py files so the package imports work. I made one small compatibility fix: PhysicalLayer.decapsulate now returns a PDU so the receiver-side decapsulation chain receives PDUs (matching the other layers).

Below are the files. Save them with the same relative paths.

```python name=osi_blockchain_simulation/main_simulation.py
#!/usr/bin/env python3
import argparse
...
```python name=osi_blockchain_simulation/core/__init__.py
# core package
```

```python name=osi_blockchain_simulation/core/pdu.py
class PDU:
...
```python name=osi_blockchain_simulation/core/crypto_utils.py
...
```python name=osi_blockchain_simulation/core/verification_utils.py
import hashlib
...
```python name=osi_blockchain_simulation/core/layers/__init__.py
# core.layers package
```

```python name=osi_blockchain_simulation/core/layers/application.py
...
```python name=osi_blockchain_simulation/core/layers/presentation.py
from core.pdu import PDU
...
```python name=osi_blockchain_simulation/core/layers/session.py
from core.pdu import PDU
...
```python name=osi_blockchain_simulation/core/layers/transport.py
from core.pdu import PDU
...
```python name=osi_blockchain_simulation/core/layers/network.py
from core.pdu import PDU
...
```python name=osi_blockchain_simulation/core/layers/datalink.py
from core.pdu import PDU
...
```python name=osi_blockchain_simulation/core/layers/physical.py
from core.pdu import PDU
...
class PhysicalLayer:
    ...
    def decapsulate(self, bits):
        ...
        # Return a PDU so upper layers receive the expected object type
        return PDU(payload=data)
```

```python name=osi_blockchain_simulation/blockchain_module/__init__.py
# blockchain_module package
```

```python name=osi_blockchain_simulation/blockchain_module/transaction.py
import time
...
```python name=osi_blockchain_simulation/blockchain_module/block.py
import hashlib
import time
...
```python name=osi_blockchain_simulation/examples/simple_text_flow.py
import sys
sys.argv = ['main_simulation.py', '--data', 'Hello OSI World']
import main_simulation
```

```python name=osi_blockchain_simulation/examples/encrypted_data_flow.py
import sys
sys.argv = ['main_simulation.py', '--data', 'Secret Message', '--encrypt']
import main_simulation
```

```python name=osi_blockchain_simulation/examples/signed_data_flow.py
import sys
sys.argv = ['main_simulation.py', '--data', 'Signed Message', '--sign']
import main_simulation
```

```python name=osi_blockchain_simulation/examples/blockchain_flow.py
import sys
...
- To run the simulation directly: from the project root run python -m osi_blockchain_simulation.main_simulation (or cd into osi_blockchain_simulation and run python main_simulation.py).
- The PhysicalLayer.decapsulate was adjusted to return a PDU so the rest of the decapsulation chain works as intended.
- If you want, I can:
  - Add unit tests,
  - Add command-line help examples,
  - Improve the encryption and signing to use proper cryptography libraries,
  - Or open a PR with these files in your repository. Which would you like next?
